...
//    dna_q = "GTAAGTACCCTATCGATG";
//    dna_d = "GTAAGTAAATCGATGTT";
...
//                            std::cout << "line 662" << std::endl;mopt
//    mopt.best_n = 1;
    mopt.flag |= MM_F_CIGAR; // DO NOT perform alignment
...
//    char reference_seq_array[referenceSeqLength + 1];
...
//                            std::cout << refSeq << std::endl << querySeq << std::endl << std::endl;
    ...
    reg = mm_map(mi, querySeqLength, query_seq_array, &n_reg, tbuf, &mopt,
                 0); // get all hits for the query
    std::cout << "line 711" << std::endl;
    int i;
    if (n_reg > 0  && (&reg[0])->rev == 0) {
        mm_reg1_t *r = &reg[0];
//        assert(r->p); // with MM_F_CIGAR, this should not be NULL
//        std::cout << std::to_string(startRef) << "\t" << std::to_string(endRef) << "\t" << alignmentName << std::endl;
