                    ...
                    if len(refali) == len(queryali):
                        ...
                        if querychr != refchr:   ## interchrosome relocations are all wrong
                            ...
                            if querystrand[0] == '+':
                                for i in range(len(refali)):
                                    if refali[i] != '-':
                                        if queryali[i] != '-' and benchMarkalignedPosition[refchr][refstart+refPosition] == (querystart + queryPosition):
                                            benchMarkalignedPosition[refchr][refstart+refPosition] = -5 # if it is same with benchmark, give it a value of -5
                                        elif queryali[i] == '-' and benchMarkalignedPosition[refchr][refstart+refPosition] == -1:
...
output = open(sys.argv[2] + ".aliAllEvaluatioin", 'w')
